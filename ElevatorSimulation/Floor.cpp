// identification comments code block
// Programmer: Ali Babaei
// Programmer's ID: 1447397

#include "Floor.h"
#include "Rider.h"
#include "Panel.h"
#include "Building.h"

#include <iostream>
#include <string>
#include <vector>
using namespace std;

#include <climits>

const char* const Floor::UP = "Up";
const char* const Floor::DOWN = "Down";
int Floor::TOP = INT_MIN; // defined in climits
int Floor::BOTTOM = INT_MAX; // also in climits

Floor::Floor(const int elevation, const char* const name, const char* const label)
:elevation(elevation), name(name), label(label)
{
  panel.addButton(UP);
  panel.addButton(DOWN);

  if (TOP < elevation) TOP = elevation;
  if (elevation < BOTTOM) BOTTOM = elevation;
}

ostream& operator<<(ostream& out, const Floor& floor)
{
  out.width(2);out << floor.label;
  out.width(13);out << floor.name;
  out << " at ";
  out.width(5); out<< floor.elevation;
  out << "\" UP/Down: ";
  out.widen(5); out << floor.upRiders.size() << "/";
  out.widen(5); out << floor.downRiders.size();
  out.widen(5); out << floor.panel;

  return out;
}

void Floor::addRider(const Rider& r)
{
  if(r.goingDown)
  {
    downRiders.push(r);
    panel.press(DOWN);
  }
  else if(r.goingUp)
  {
    upRiders.push(r);
    panel.press(UP);
  }
  else cout << "error 1";
}

Rider Floor::removeUpRider( )
{
  if (upRiders.empty()) throw " up riders is empty";

  Rider r = upRiders.front();
  upRiders.pop();

  return r;
}

Rider Floor::removeDownRider( )
{
  if (downRiders.empty()) throw "down riders is empty";

  Rider r = downRiders.front();
  downRiders.pop();

  return r;
}
